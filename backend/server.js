...
// CRITICAL: Manual CORS handling for Vercel
...
app.get('/api/emissions/calculate', async (req, res) => {
  try {
    ...
    const factor = emissionFactors[transportMode] || 0.2;
    const emissions = distance * factor;
    const alternativeEmissions = emissions * 0.3;
...
if (require.main === module) {
  app.listen(PORT, () => {
    ...
    console.log(`🌐 Environment: ${process.env.NODE_ENV || 'development'}`);
    console.log(`🔧 CORS enabled manually for all origins`);
  });
}